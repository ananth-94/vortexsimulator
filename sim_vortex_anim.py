...
class MyScene(Scene):
	def construct(self):
		Title = "Progression of a standard glitch"
		Subtitle = "20240526/set1/run1 - 1357 to 1361"
		...
		t_start = 1357 # Start time of the glitch/feature of interest
		t_end = 1361 # End time of the glitch/feature of interest

		# Reading the complete output data obtained from the simulation (sim_vortex_pos.dat or sim_vortex_pos_minimal.dat)
		data = pd.read_csv(r'D:\Codes\Vortex\runs\20240526\set1\run1\sim_vortex_pos_minimal.dat', delimiter='\t')
		omegacvst = pd.read_csv(r'D:\Codes\Vortex\runs\20240526\set1\run1\sim_omega_c.dat', delimiter='\t')
		...
		# A vortex is considered to move if it changes position by a distance greater than eepsilon, in the given slice of time
		...
		for t in range(1,time_tot,1):
			...
			# *10 is used to reconcile the rootation rate data with the decimated position data
