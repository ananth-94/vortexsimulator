...
His patience and incisive comments provided the scaffold upon which this work was built.
...
Output of interest is to be archived in 'runs' for temporary storage. An example archive has been left in place.
The code for animation is placed in 'animation'.
A jupyter notebook for analysis, 'template.ipynb', is available in 'notebooks'.

# Running the script -- Requires 'gcc'
The parameters of the simulation are set directly in the 'parameters.h' file available in the 'codes' folder.
Move to the root folder in terminal and run the following command
./runscript.sh


# Accessing the analysis
Move to the 'notebooks' folder in terminal and run the following command
jupyter lab
...
The animation code for 2000 vortices and 5000 vortices are saved as different files.
...
# Usage -- Analysis code -- Requires python packages 'jupyter', 'pandas', 'matplotlib', 'numpy', 'seaborn'
