...
class MyScene(Scene):
	def construct(self):
		Title = "Progression of a spontaneous glitch in a star with 5000 vortices and strong pinning"
		Subtitle = "20260401/setA/run1 - 1597.3 to 1598.5"
		...
		t_start = 1597.3 # Start time of the glitch/feature of interest
		t_end = 1598.5 # End time of the glitch/feature of interest

		# Reading the complete output data obtained from the simulation (sim_vortex_pos.dat or sim_vortex_pos_minimal.dat)
		data = pd.read_csv(r'/mnt/usb-Realtek_RTL9210_NVME_012345679989-0:0-part2/backup_work/vortex/runs/20260401/setA/run1/sim_vortex_pos.dat', delimiter='\t')
		omegacvst = pd.read_csv(r'/mnt/usb-Realtek_RTL9210_NVME_012345679989-0:0-part2/backup_work/vortex/runs/20260401/setA/run1/sim_omega_c.dat', delimiter='\t')
		...
		for dot in dots_moving:
			trace.append(TracedPath(dot.get_center, dissipating_time=0.2, stroke_opacity=[0, 1]))
		self.add(*trace)
		...
		for t in range(1,time_tot,1):
			...
			# t*10 is used to reconcile the rotation rate data with the decimated position data
			# t*5 is used to reconcile the rotation rate data with the reduced position data
