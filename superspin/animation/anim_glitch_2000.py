...
# Open terminal and to a folder of your choice
...
class MyScene(Scene):
	def construct(self):
		Title = "Progression of a glitch in a star with 2000 vortices and 78 traps"
		Subtitle = "20250623/set1/run1 - 418.5 to 421.5"
		...
		t_start = 226.5 # Start time of the glitch/feature of interest
		t_end = 230.5 # End time of the glitch/feature of interest

		# Reading the complete output data obtained from the simulation (sim_vortex_pos.dat or sim_vortex_pos_minimal.dat)
		data = pd.read_csv(r'/mnt/usb-Realtek_RTL9210_NVME_012345679989-0:0-part2/backup_work/vortex/runs/20250623/set1/run1/sim_vortex_pos_minimal.dat', delimiter='\t')
		omegacvst = pd.read_csv(r'/mnt/usb-Realtek_RTL9210_NVME_012345679989-0:0-part2/backup_work/vortex/runs/20250623/set1/run1/sim_omega_c.dat', delimiter='\t')
		...
		for t in range(1,time_tot,1):
			...
			# Graph
			graph = ax.plot_line_graph(omegacvst_select[0:(t*5),0], omegacvst_select[0:(t*5),1], add_vertex_dots=False, line_color = BLUE)
			self.add(graph)

			# *10 is used to reconcile the rotation rate data with the decimated position data
			# *5 is used to reconcile the rotation rate data with the reduced position data
