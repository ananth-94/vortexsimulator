...
vector<double> triggers_init()
{
	...
	{
		//times_trig[i] = randnumb01() * dynamics_runtime; // Random times
		times_trig[i] = (dynamics_runtime/(number_triggers+1)) * (i+1); // Evenly spaced times
	}
...
vector<vector<double>> pinning_vector_copy(nmax, vector<double>(nmax, 0.0)); // DEBUG
pair<int, int> triggers_check(double t, vector<vector<double>> &pinning_vector, int &count_trig, int &state_trig, vector<double> &times_trig)
{
	...
	if (trigger_type != "OFF")
	{
		...
		if (t > times_trig[count_trig] && state_trig == 0)
		{
			...
			pinning_vector_copy = pinning_vector; // DEBUG
