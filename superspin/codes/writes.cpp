...
void write_progress(double t, int &progress, const double runtime, const string prefix)
{
	if ( (t/runtime)*100 >= progress+1)
	{
		...
		// To print the progress of dynamics to terminal
		if(prefix == "Dynamics")
		{
			cout << "\rProgress: " << setw(3) << setfill('0') << progress;
			cout.flush();			
		}
	}
}
...
void write_info_start(vector<double> &f)
{
	...
	outfile << "I_c = " << I_c << endl;
	outfile << "I_s = " << I_s << endl;
	// System derived
	...
	outfile << "A_ext = " << A_ext/(omega_0/T_0) << " omega_0/T_0" << endl;
	// Dynamics derived
	outfile << "// Dynamics derived" << endl;
	outfile << "N_ext = " << N_ext << endl << endl;
  ...
  outfile << "error_tolerance = " << error_tolerance << endl;
	// Barnes-Hut
	...
	// Writing
	outfile << "// Writing" << endl;
	outfile << "write_step = " << write_step << endl << endl;
...
void write_data_post_dynamics(vector<double> &t_vector, vector<double> &t_vector_write, vector<vector<double>> &f_vector, vector<double> &omega_c_vector, vector<double> &omega_s_vector,
	vector<int> &number_off_vector, vector<int> &sector_id_vector, vector<double> &trigger_times, vector<int> &out_vector, vector<int> &unpinned_vector, int run_id)
{
	fstream outfile;
	...
	// Write position data only for the first run
	if (run_id == 1)
	{
		// Full positions
		filename = "sim_vortex_pos.dat"; 
		fullpath = pathplace+filename;
		
		outfile.open(fullpath, ios::out | ios::app);
		outfile.precision(15);
		
		for (int i=0; i<t_vector_write.size(); i++)
		{
			outfile << t_vector_write[i];
